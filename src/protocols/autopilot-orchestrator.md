...
11.5. **Cross-contamination detection (VRFY-06):** If `pipeline_steps.verify.agent_spawned` is `true`:
    - Read `.planning/phases/{phase}/VERIFICATION.md` and scan for executor-referencing contamination patterns (case-insensitive). Direct patterns: "executor reported", "executor's confidence", "executor claimed", "according to the executor", "executor's self-assessment", "executor determined", "executor concluded". Paraphrased patterns: "executor indicated", "executor noted", "executor stated", "executor mentioned", "executor found that", "based on executor", "from the executor", "per the executor", "executor's findings", "executor's results", "executor's output".
    - Read `.planning/phases/{phase}/JUDGE-REPORT.md` and scan for verifier-conclusion-referencing contamination patterns (case-insensitive). Direct patterns: "verifier concluded", "verifier passed", "verifier failed", "verifier's score", "verification result was", "verifier determined", "verifier rated". Paraphrased patterns: "verifier indicated", "verifier noted", "verifier stated", "verifier mentioned", "verifier found that", "based on verifier", "from the verifier", "per the verifier", "verifier's findings", "verifier's conclusion", "verifier's assessment", "verifier reported that", "verifier gave", "verifier scored", "verifier awarded".
    - Read `.planning/phases/{phase}/SCORECARD.md` and scan for verifier/judge-referencing contamination patterns (case-insensitive). Direct patterns: "verifier found", "judge recommended", "judge's concerns", "verification report shows", "judge concluded", "verifier reported", "judge determined", "verifier's assessment". Paraphrased patterns: "judge indicated", "judge noted", "judge stated", "judge mentioned", "judge found that", "based on judge", "from the judge", "per the judge", "judge's findings", "judge's recommendation", "judge's conclusion", "verification indicated", "judge approved", "judge rejected", "verifier noted", "verifier indicated".
    - **False-positive exclusion:** Before flagging a match, check the surrounding context. Matches within structural/procedural references (e.g., "the verifier agent is spawned", "spawn the judge", "verifier prompt", "after the verifier returns", "the judge reads") are NOT contamination -- they are legitimate process references. Only flag matches that reference another agent's conclusions, findings, or scores.
    - If any contamination pattern is found (after false-positive exclusion) in any file: Log WARNING: "Cross-contamination detected in {file}: pattern '{matched}' found. Agent independence may be compromised." Append `cross_contamination_warning` event to event_log with details (file, pattern, line number). This is a WARNING, not a REJECT -- the phase-runner's contamination detection is the primary enforcement; this orchestrator check is a secondary safety net.
