...
8. **Action field quality check (PROM-02):** Reject any task whose action field is a vague stub. An action field MUST contain specific implementation steps (which functions/sections to modify, what content to add, where to make changes). Flag action fields shorter than 20 words or containing only generic phrases like "implement X", "add Y feature", "update Z" without specifying HOW as a **blocker** with: `{"severity": "blocker", "description": "Task {id} action field is a stub -- lacks specific implementation steps.", "fix_hint": "Expand the action field to specify which functions to add/modify, which sections to edit, and what content to write."}`.
9. Return structured JSON with pass/fail, issues, and confidence score
</must>
...
> 6. Write task completion status to .planning/phases/{phase}/EXECUTION-LOG.md IMMEDIATELY after completing each task -- before starting the next task. Each entry must include: task ID, status (COMPLETED/FAILED/NEEDS_REVIEW), commit SHA, files modified, evidence summary, test_results ({PASS count}/{total count} assertions passed), elapsed time for the task (record start and end timestamps, compute duration), and a confidence score (1-10). Scoring guide: 10=all criteria verified with evidence, 7-9=criteria verified but minor concerns, 4-6=some criteria unverified or uncertain, 1-3=significant issues remain. If your confidence is below 7, set status to NEEDS_REVIEW -- the phase-runner will spawn a mini-verification agent before you proceed to the next task. Do NOT batch these writes.
...
**Contamination Check (VERIFY):** After receiving the verifier's return, scan all text fields in the return JSON (criteria_results[].evidence, failures[], scope_creep[]) for verifier contamination markers defined in the phase-runner's `cross_contamination_detection` section. This includes both direct patterns (e.g., "executor reported", "executor's confidence", "executor claimed") and paraphrased patterns (e.g., "executor indicated", "executor noted", "executor stated", "based on executor", "executor's findings"). Before flagging a match, apply the false-positive exclusion check: verify the match is not within a safe structural/procedural context (e.g., "the verifier agent is spawned" or "verifier prompt"). If contamination is detected, reject the return and re-spawn the verifier with a clean prompt plus contamination warning. Max 1 contamination re-spawn per agent.
...
> </should>
>
> **Silent operation (PROM-03):** Gather evidence and read files silently without narrating each action. Present your independent findings directly.
>
> <may>
...
**Contamination Check (JUDGE):** After receiving the judge's return, scan all text fields in the return JSON (concerns[], independent_evidence[], notes, verifier_missed[]) for judge contamination markers defined in the phase-runner's `cross_contamination_detection` section. This includes both direct patterns (e.g., "verifier concluded", "verifier passed", "verifier's score") and paraphrased patterns (e.g., "verifier indicated", "verifier noted", "verifier stated", "based on verifier", "verifier's findings", "verifier's conclusion"). Before flagging a match, apply the false-positive exclusion check: verify the match is not within a safe structural/procedural context (e.g., "spawn the judge" or "after the verifier returns"). If contamination is detected, reject the return and re-spawn the judge with a clean prompt plus contamination warning. Max 1 contamination re-spawn per agent.
...
> </may>
>
> **Silent operation (PROM-03):** Run verification commands and read files silently without narrating each action. Present your scorecard directly.
>
...
**Contamination Check (RATE):** After receiving the rating agent's return, scan all text fields in the return JSON (scorecard[].justification, scorecard[].evidence, aggregate_justification, side_effects[]) for rating contamination markers defined in the phase-runner's `cross_contamination_detection` section. This includes both direct patterns (e.g., "verifier found", "judge recommended", "judge's concerns", "verification report shows") and paraphrased patterns (e.g., "judge indicated", "judge noted", "judge stated", "based on judge", "judge's findings", "verification indicated", "judge approved", "judge rejected"). Before flagging a match, apply the false-positive exclusion check: verify the match is not within a safe structural/procedural context (e.g., "rating agent prompt" or "the judge reads"). If contamination is detected, reject the return and re-spawn the rating agent with a clean prompt plus contamination warning. Max 1 contamination re-spawn per agent.
