...
- `--gaps [phase]` — analyze and resolve the specific deficiencies preventing a completed phase from reaching 10/10; produces ordered list of remaining issues, then executes micro-targeted fixes one deficiency at a time, working toward 9.5+/10; max 5 gap-fix iterations; cannot combine with `--complete` (gaps targets completed phases, complete targets incomplete phases); can combine with `--quality` (quality runs first to 9.5, then gaps pushes higher)
...
- `--gaps` cannot combine with `--complete` (gaps targets completed phases, complete targets incomplete phases)
